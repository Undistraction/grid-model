import createDimensions, {
  ...
  INCORRECT_NUMBER_OF_PARAMS_MESSAGE,
} from '../dimensions';

describe('dimensions', () => {
  describe('invalid params', () => {
    it('throws an error if only width, only height, or only aspectRatio are supplied', () => {
      expect(() => createDimensions({ width: 100 })).toThrowError(
        new RegExp(INCORRECT_NUMBER_OF_PARAMS_MESSAGE)
      );
  ...
  it("throws error if width or height aren't valid numbers or percentages", () => {
    ...
    expect(() => createDimensions({ width: 33, height: '10' })).toThrowError(
      new RegExp(INVALID_PARAMS_MESSAGE)
    );
  });
